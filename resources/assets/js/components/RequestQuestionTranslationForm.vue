...
export default {
  ...
  methods: {
    ...
    onSubmit() {
      ...
      http.post('/api/questions/' + this.question.id +'/translation_requests/', this.payload)
        .then(response => {
  ...
  updated() {
    if (this.user.languages.length > 0) {
      $('#requestQuestionTranslationForm [name=language]').dropdown('set selected', this.user.languages[0].code)
    }
  }
}
</script>
